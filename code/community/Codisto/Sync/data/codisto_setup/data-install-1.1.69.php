...
if(!isset($MerchantID) || !isset($HostKey))
{
	...
	if(!preg_match('/\/codisto-sync\//', $path))
	{
		...
		catch(Exception $e)
		{
			...
			//If competing requests are coming in as the exttension is installed the lock above will be held ... don't report this back to Codisto .
			if($e->getCode() != "HY000") {
			...
			if($createMerchant)
			{
				...
				try
				{
					...
					if(!isset($MerchantID) || !isset($HostKey))
					{
						...
						$client->setHeaders('Content-Type', 'application/json');ab -n 10 -c5
						{
							try
							{
